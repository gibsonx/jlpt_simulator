...
- Ensure all readings follow valid Japanese pronunciation rules (no invented or impossible readings)
- Assuming that word a consists of 2-3 characters, set them in order as x[0],y[0],z[0] (if a variable is a Japanese kanji, do not display it as hiragana pronunciation). 
Generate 4 options: Option 1–4 in format x[i], y[i], z[i] (i = 1–4)
If x[0] is hiragana → set x[1]=x[2]=x[3]=x[4]=x[0]
If y[0] is hiragana → set y[1]=y[2]=y[3]=y[4]=y[0]
If z[0] is hiragana → set z[1]=z[2]=z[3]=z[4]=z[0]
...
- Don't show question instructions and sequence number in the generated content. 
- The word in the sentence can neither be used in the question nor options.
- You must show the correct answer in the output, for example: 正解: n . By referring the correct answer(c) in the "Historical Generation" during the question generation, you should ensure that the selection of correct answers (1, 2, 3, or 4) remains randomly distributed, not always show as: 1
...
<a>このメモを読んで、ミンさんはまず何をしなければならないか。</a>  
<ul class='options'>
...
<div class='article'>
    ...
    <a>「海外のファッションの会社」がしていることとして、合っているのはどれか。</a>  
    <ul class='options'>  
    ...
    <a>漁師たちは、なぜ「海外のファッションの会社」が始めた活動に参加するのか。</a>  
    <ul class='options'>
    ...
    <a>この文章を書いた人は、日本で売られ始めた「海外のファッションの会社」の服や靴について、どのように考えているか。</a>  
    <ul class='options'>  
...
you must provide 2 html format tables with several lines and append additional information for retrieve below. The content and conditions combined should be more than 300 words and complex enough for JLPT n3 level.
...
<a>次のうち、正しい活動の選択肢はどれか。（※問題文の具体的な選択肢が不足しているため、活動内容から推測）</a>
<ul class="options">
...
<a>瞬時活動の魅力者になりたい人が気をつけるべきことはどれか。</a>
    <ul class="options">
      <li>機能の活躍に応募できない</li>
